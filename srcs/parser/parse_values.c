...
{
	if (!parse_vec3(str, v) || vec3_len(*v) != 1.0) // 精度心配。本当は厳密に1なのにdouble計算によて1ではなくなる恐れがある。
		return (false);
	return (true);
}
...
{
	if (!parse_int(str, &color->r) || !parse_int(str, &color->g) || !parse_int(str, &color->b))
		return (false);
...
bool	parse_fov(const char *str, double *fov)
{
	...
	if (!parse_int(str, &fov_int) || fov_int < 0 || fov_int > 1)
		return (false);	
	*fov = (double)fov_int;
	return (true);
}
